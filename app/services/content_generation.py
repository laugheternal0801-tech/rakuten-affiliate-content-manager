...
class LLMContentGenerator(ContentGenerator):
    ...
    def _user_prompt(context: GenerationContext, reference_data: dict[str, Any]) -> str:
        ...
        return (
            ...
            f"比較する商品：参照データのproductsにある{product_count}点\n"
            f"想定読者：{context.target_audience}\n"
            f"狙うキーワード：{context.main_keyword}\n"
            "分量：本文全体で2,600〜3,200字。3,200字を超えないこと。\n\n"
            "構成\n"
            "1. 読者がいま困っている場面の描写\n"
            "2. この記事が向いていない人（先に外す）\n"
            "3. 選ぶときに見るべき基準を3つ\n"
            ...
            "7. まとめ\n\n"
            "文字数の必須配分\n"
            "・1の導入は180〜220字、2の向いていない人は100〜140字にする。\n"
            "・3の選ぶ基準は3項目の合計で300〜360字にする。\n"
            "・4の縦型比較表は、商品名の小見出しを含めて1商品120字以内にする。\n"
            f"・5の商品レビューは1商品{review_char_limit}字以内にする。"
            "良い点・気になる点・向いている人を簡潔にまとめる。\n"
            "・6の使い方別おすすめは200〜240字、7のまとめは140〜180字にする。\n"
            "・同じ価格・評価・特徴を、導入、比較表、レビュー、まとめで何度も繰り返さない。\n"
